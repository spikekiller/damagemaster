...
function Damagelog:DrawOldLogs()
	...
	self.OldLogs.UpdateDates = function()
		for year,months in pairs(self.CurLogsTable) do
			local year_node = self.DateChoice:AddNode("20"..year)
			...
			for month,days in pairs(months) do
				...
				for day,maps in pairs(days) do
					...
					for map,map_infos in pairs(maps) do
						local f = string.find(map, "_")
	...
	player_info:SetName("Player informations")
	...
	self.DamageInfoForm:SetName("Damage informations section")
	...
	self.OldDamageInfo:AddColumn("Damage informations")
	...
	self.DateChoice.OnNodeSelected = function(panel, selected)
		if selected.year and selected.month and selected.day and selected.map and selected.text then
			self.RoundChoice:Clear()
			local infos = Damagelog.CurLogsTable[selected.year][selected.month][selected.day][selected.map]
			round_column:SetName("Rounds of "..selected.day.."/"..selected.month.."/"..selected.year.." - "..selected.text)
			for i=1, infos.rounds do
				self.RoundChoice:AddLine("Round "..tostring(i)).Round = i
			end
			self.CurSelectedRound = {
				year = selected.year,
	...
	self.LoadLogs.DoClick = function(panel)
		...
		elseif panel.Bot then
			if not self.CurSelectedRound then
				Derma_Message("Select a map to load!", "Error", "OK")
				return
			end
			...
			net.WriteTable(self.CurSelectedRound)
			net.WriteUInt(self.RoundChoice:GetSelected()[1].Round, 8)
			net.SendToServer()
			panel.MoveTop = true
		end
	end
